  ...
  new GoogleStrategy(
    ...
    async function (accessToken, refreshToken, profile, done) {
      try {
        const existingUser = await User.findOne().or([
          { email: profile.emails[0].value },
          { firstName: profile.name.givenName }, // TODO Check if existing name "starts with" instead of equals profile first name
          { lastName: profile.name.familyName }, // TODO Check if existing last "starts with" instead of equals profile last name
        ]);
