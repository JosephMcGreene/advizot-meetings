...
import {
  ...
  getOneUserByID,
  saveNewUser,
} from "../controllers/users.queries.js";
...
passport.deserializeUser(async (id, done) => {
  const userID = await getOneUserByID(id);
  ...
  new LinkedInStrategy(
    ...
    async function (accessToken, refreshToken, profile, done) {
      try {
        ...
        const newUser = saveNewUser(profile, "linkedinID");
  ...
  new GoogleStrategy(
    ...
    async function (accessToken, refreshToken, profile, done) {
      try {
        ...
        const newUser = saveNewUser(profile, "googleID");
        return done(null, newUser);
      } catch (err) {
        done(err);
        throw err;
